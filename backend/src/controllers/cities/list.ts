...
export const listCities = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (long && lat) {
    ...
    queryBuilder
      .addSelect('ST_Distance(city.coordinates, ST_SetSRID(ST_GeomFromGeoJSON(:origin), 4326)) AS distance')
  ...
  try {
    ...
    const customError = new CustomError(400, 'Raw', `Can't retrieve list of cities.`, null, err);
