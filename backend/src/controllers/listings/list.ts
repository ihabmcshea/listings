...
export const showListings = async (
  ...
  try {
    if (city && !(long && lat && radius)) {
      ...
      const listingsWithDistance = data.entities.map(
        (listing: ListingWithDistance, index: number) => ({
          ...listing,
...
export const showMyDrafts = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const getMyDraftCount = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    const customError = new CustomError(
      400,
      "Raw",
      "Error retrieving draft listings count",
      null,
      err
    );
    return next(customError);
